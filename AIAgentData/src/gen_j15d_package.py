...
MODEL_NAME = "J-15D_Roaring_Shark"
MODEL_NAME_I18N = "J-15D咆哮鲨"
IMAGE_QUERY = "Shenyang J-15D electronic warfare aircraft side view"
...
AGENT_DESC = "中制舰载电子战飞机，具备电子压制、伴随护航、干扰压制与舰队电磁支援能力。"
PREFERRED_IMAGE_URLS = [
    "https://static.turbosquid.com/Preview/2019/06/28__07_16_05/J15FlyingSharkn1.jpg43A446D6-56C5-427A-91AF-841C62BA62B4Default.jpg",
    "https://static.turbosquid.com/Preview/2019/06/28__07_16_05/J15FlyingSharkn6.jpg3F274D0D-25D4-40B3-A5A4-0722F7E1357DDefaultHQ.jpg",
    "https://static.turbosquid.com/Preview/2014/05/20__22_07_53/J15_FShark_White_Cam03.jpg4ab0a193-3fae-4769-9897-0ec97401f6c0Zoom.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("j-15", "j15", "flying shark", "fighter", "turbosquid", "shark")
    ...
    if not candidates:
        raise RuntimeError("No Bing image candidates found for J-15D.")

    for url in candidates[:80]:
        consider(url)

    if best is None:
        raise RuntimeError("Failed to download a valid real J-15D thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for J-15D.")
...
def build_transport_mesh():
    ...
    fuselage = trimesh.creation.capsule(radius=0.32, height=5.7, count=[24, 24])
    ...
    nose = trimesh.creation.cone(radius=0.24, height=0.95, sections=24)
    nose.apply_transform(trimesh.transformations.rotation_matrix(-np.pi / 2, [0, 1, 0]))
    nose.apply_translation([3.25, 0, 0])
    nose.visual.face_colors = body_color
    parts.append(nose)

    wing = trimesh.creation.box(extents=[1.35, 5.8, 0.12])
    wing.apply_transform(trimesh.transformations.rotation_matrix(np.deg2rad(-18), [0, 0, 1]))
    wing.apply_translation([0.15, 0, 0.05])
    wing.visual.face_colors = body_color
    parts.append(wing)

    stabilizer = trimesh.creation.box(extents=[0.65, 2.2, 0.08])
    stabilizer.apply_transform(trimesh.transformations.rotation_matrix(np.deg2rad(-22), [0, 0, 1]))
    stabilizer.apply_translation([-2.55, 0, 0.55])
    stabilizer.visual.face_colors = body_color
    parts.append(stabilizer)

    for side in (-1, 1):
        v_tail = trimesh.creation.box(extents=[0.65, 0.1, 0.95])
        v_tail.apply_transform(trimesh.transformations.rotation_matrix(np.deg2rad(15 * side), [1, 0, 0]))
        v_tail.apply_translation([-2.9, side * 0.42, 0.88])
        v_tail.visual.face_colors = body_color
        parts.append(v_tail)

    for side in (-1, 1):
        eng = trimesh.creation.cylinder(radius=0.2, height=1.1, sections=24)
        eng.apply_transform(trimesh.transformations.rotation_matrix(np.pi / 2, [0, 1, 0]))
        eng.apply_translation([0.15, side * 0.72, -0.16])
        eng.visual.face_colors = dark
        parts.append(eng)

    for side in (-1, 1):
        pod = trimesh.creation.cylinder(radius=0.08, height=0.7, sections=20)
        pod.apply_transform(trimesh.transformations.rotation_matrix(np.pi / 2, [1, 0, 0]))
        pod.apply_translation([0.95, side * 2.82, 0.03])
        pod.visual.face_colors = dark
        parts.append(pod)
...
def export_fallback_glb(output_glb):
    ...
    # The fallback fighter mesh is authored directly in the LaViC aircraft canonical pose.
...
resp = server.create_rodin_job(
    text_prompt='Shenyang J-15D carrier-based electronic warfare aircraft, realistic hard-surface twin-engine naval fighter jet, full plane, wingtip electronic warfare pods, high quality military aircraft',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for J-15D.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] J-15D package generated successfully.")
