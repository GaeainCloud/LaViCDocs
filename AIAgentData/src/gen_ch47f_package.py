...
MODEL_NAME = "CH-47F_Chinook"
MODEL_NAME_I18N = "CH-47F直升机"
IMAGE_QUERY = "CH-47F Chinook 3d render side view"
...
AGENT_DESC = "美制重型运输直升机，具备兵力投送、重装运输与多用途支援能力。"
PREFERRED_IMAGE_URLS = [
    "https://www.renderhub.com/pxfpxd/boeing-ch-47-chinook/boeing-ch-47-chinook-06.jpg",
    "https://static.turbosquid.com/Preview/001215/665/4X/3D-heavy-helicopter-ch-47-chinook-model_D.jpg",
    "https://preview.free3d.com/img/2023/01/3298979082007480029/czmizc3m.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    if not candidates:
        raise RuntimeError("No Bing image candidates found for CH-47F.")

    best = None
    good_kw = ("ch-47f", "chinook", "boeing", "helicopter", "transport")
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid real CH-47F thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for CH-47F.")
...
def build_transport_mesh():
    ...
    fuselage = trimesh.creation.capsule(radius=0.5, height=8.8, count=[24, 24])
    ...
    nose = trimesh.creation.icosphere(subdivisions=2, radius=0.5)
    nose.apply_scale([1.5, 1.0, 0.9])
    nose.apply_translation([4.3, 0.0, 0.05])
    nose.visual.face_colors = body_color
    parts.append(nose)

    rear_pylon = trimesh.creation.box(extents=[1.0, 0.9, 1.4])
    rear_pylon.apply_translation([-3.9, 0.0, 0.95])
    rear_pylon.visual.face_colors = body_color
    parts.append(rear_pylon)

    front_pylon = trimesh.creation.box(extents=[0.8, 0.8, 1.1])
    front_pylon.apply_translation([2.3, 0.0, 0.85])
    front_pylon.visual.face_colors = body_color
    parts.append(front_pylon)

    rear_ramp = trimesh.creation.box(extents=[1.1, 1.0, 0.08])
    rear_ramp.apply_transform(trimesh.transformations.rotation_matrix(np.radians(22), [0, 1, 0]))
    rear_ramp.apply_translation([-4.95, 0.0, -0.4])
    rear_ramp.visual.face_colors = body_color
    parts.append(rear_ramp)

    h_tail = trimesh.creation.box(extents=[0.5, 2.6, 0.1])
    h_tail.apply_translation([-4.3, 0.0, 1.55])
    h_tail.visual.face_colors = body_color
    parts.append(h_tail)

    v_tail = trimesh.creation.box(extents=[0.25, 0.12, 1.1])
    v_tail.apply_translation([-4.35, 0.0, 1.95])
    v_tail.visual.face_colors = body_color
    parts.append(v_tail)

    sponson_l = trimesh.creation.box(extents=[2.2, 0.55, 0.5])
    sponson_l.apply_translation([0.3, 1.1, -0.2])
    ...
    for mast_x, mast_z in [(2.3, 1.5), (-3.7, 1.75)]:
        mast = trimesh.creation.cylinder(radius=0.08, height=0.5, sections=20)
        mast.apply_translation([mast_x, 0.0, mast_z])
        mast.visual.face_colors = dark
        parts.append(mast)

        for angle in [0, np.pi / 2, np.pi, 3 * np.pi / 2]:
            blade = trimesh.creation.box(extents=[4.2, 0.16, 0.03])
            blade.apply_translation([mast_x + 2.1, 0.0, mast_z + 0.25])
...
resp = server.create_rodin_job(
    text_prompt='CH-47F Chinook military transport helicopter, realistic hard-surface tandem-rotor helicopter, full aircraft, front and rear rotors, high quality heavy-lift military helicopter',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for CH-47F.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] CH-47F package generated successfully.")
