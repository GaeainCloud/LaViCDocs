...
MODEL_NAME = "Z-20_Helicopter"
MODEL_NAME_I18N = "直-20直升机"
IMAGE_QUERY = "Harbin Z-20 helicopter 3d render side view"
...
AGENT_DESC = "中国中型通用运输直升机，具备机降突击、人员输送与多用途支援能力。"
PREFERRED_IMAGE_URLS = [
    "https://img-new.cgtrader.com/items/4764714/b8cc6082bf/chinese-z20-helicopter-3d-model-b8cc6082bf.jpg",
    "https://img-new.cgtrader.com/items/4764714/8a84f79de1/chinese-z20-helicopter-3d-model-8a84f79de1.jpg",
    "https://p.turbosquid.com/ts-thumb/fi/J52d8R/VC/z20xk_0000/jpg/1696996781/1920x1080/fit_q87/e780fdf1f596c9356bdc8668a171805bcd42fe69/z20xk_0000.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    if not candidates:
        raise RuntimeError("No Bing image candidates found for Z-20.")

    best = None
    good_kw = ("z-20", "z20", "helicopter", "harbin", "utility")
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid real Z-20 thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for Z-20.")
...
resp = server.create_rodin_job(
    text_prompt='Harbin Z-20 military utility helicopter, realistic hard-surface helicopter, full aircraft, modern utility rotorcraft, main rotor and tail rotor, high quality military helicopter',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for Z-20.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] Z-20 package generated successfully.")
