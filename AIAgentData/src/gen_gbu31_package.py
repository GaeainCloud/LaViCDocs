...
MODEL_NAME = "GBU_31_JDAM"
MODEL_NAME_I18N = "GBU-31JDAM"
IMAGE_QUERY = "GBU-31 JDAM bomb side view render"
...
AGENT_DESC = "GBU-31 JDAM制导炸弹模型，采用目标跟踪导引动力学用于对地精确打击仿真。"
ENABLE_QUALITY_GATE = False
PREFERRED_IMAGE_URLS = [
    "https://static.turbosquid.com/Preview/2019/11/09__10_57_51/GBU31V4B_Art_001.png0855874D-9B8A-4BA1-B216-2CAB152AC7B3Default.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("df-15", "df15", "gbu31", "dongfeng", "missile", "ballistic", "srbm")
    bad_kw = (
        "logo",
        "patch",
        "wallpaper",
    ...
    def consider(url, strict_size=True):
        ...
        try:
            ...
            if (not strict_size) and max(w, h) < 500:
                return
            if "dongfeng-15b.jpg" in lower:
                crop = img.crop((420, 560, 4100, 1900))
                if crop.size[0] > 1000 and crop.size[1] > 300:
                    img = crop
                    w, h = img.size
            ratio = w / max(h, 1)
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid GBU-31 JDAM thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for GBU-31 JDAM missile.")
...
resp = server.create_rodin_job(
    text_prompt='GBU-31 JDAM short range ballistic missile, complete connected missile body, guided bomb, complete connected bomb body, single JDAM bomb, ogive nose, cylindrical bomb body, rear tail kit fins, no launcher, no detached parts, no floating geometry, clean topology, clean side profile',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for GBU-31 JDAM missile.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] GBU-31 JDAM package generated successfully.")
