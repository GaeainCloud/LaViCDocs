...
MODEL_NAME = "HJ_12_Missile"
MODEL_NAME_I18N = "HJ-12导弹"
IMAGE_QUERY = "HJ-12 missile side view render"
...
AGENT_DESC = "HJ-12导弹模型，采用目标跟踪导引动力学用于精确打击仿真。"
ENABLE_QUALITY_GATE = False
PREFERRED_IMAGE_URLS = [
    "https://upload.wikimedia.org/wikipedia/commons/f/f7/Red_Arrow_12_missile_at_IDEX_2017.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("hj-12", "hj12", "red arrow 12", "anti-tank", "missile", "guided missile")
    bad_kw = (
        "logo",
        "patch",
        "wallpaper",
    ...
    def consider(url, strict_size=True):
        ...
        try:
            ...
            if (not strict_size) and max(w, h) < 500:
                return
            if "red_arrow_12_missile_at_idex_2017.jpg" in lower:
                crop = img.crop((40, 120, 1440, 980))
                if crop.size[0] > 900 and crop.size[1] > 400:
                    img = crop
                    w, h = img.size
            ratio = w / max(h, 1)
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid HJ-12 thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for HJ-12 missile.")
...
resp = server.create_rodin_job(
    text_prompt='HJ-12 short range ballistic missile, complete connected missile body, anti-tank guided missile, complete connected missile body, slim cylindrical fuselage, pointed nose cone, compact mid body canards, rear tail fins, clean single missile, no launcher truck, no detached parts, no floating geometry, clean topology, clean side profile',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for HJ-12 missile.")
...
def generate_agent_json(output_json):
    ...
    agent["agentKeyword"] = "hj12_missile"
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] HJ-12 package generated successfully.")
