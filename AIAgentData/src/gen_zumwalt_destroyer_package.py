...
MODEL_NAME = "Zumwalt_Class_Destroyer"
MODEL_NAME_I18N = "朱姆沃尔特级驱逐舰"
IMAGE_QUERY = "Zumwalt class destroyer 3d render side view"
...
AGENT_DESC = "英国朱姆沃尔特级驱逐舰，具备区域防空、反导预警与编队护航作战能力。"
PREFERRED_IMAGE_URLS = [
    "https://static.turbosquid.com/Preview/2014/05/19__22_23_51/Zumwalt_Class_Destroyer_00.jpg21bb3f5b-dad7-4f9b-a7da-df6dfdcd18d0Original.jpg",
    "https://cdn.renderhub.com/ennuishao/zumwalt-class-destroyer/zumwalt-class-destroyer-05.jpg",
    "https://p.turbosquid.com/ts-thumb/2E/ERJLG5/RYpVekpV/zumwalt_render_17/jpg/1346218678/1200x1200/fit_q99/32d8d8c653f9de39a21e28abb06bfcfd3bacfc0d/zumwalt_render_17.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("zumwalt", "ddg-1000", "destroyer", "stealth ship", "turbosquid", "renderhub", "render")
    ...
    def consider(url, strict_size=True):
        ...
        try:
            ...
            if "zumwalt-class-destroyer" in lower or "zumwalt_render_17" in lower or "zumwalt" in lower:
                score += 300000
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid Zumwalt destroyer thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for Zumwalt destroyer.")
...
def build_carrier_mesh():
    ...
    hull = trimesh.creation.box(extents=[8.2, 0.82, 0.95])
    hull.apply_translation([0.0, 0.0, 0.15])
    hull.visual.face_colors = hull_color
    parts.append(hull)

    bow = trimesh.creation.box(extents=[1.25, 0.72, 0.74])
    ...
    gun.apply_translation([3.55, 0.0, 0.9])
...
resp = server.create_rodin_job(
    text_prompt='Zumwalt class destroyer, full ship, complete connected hull, stealth tumblehome destroyer, angular superstructure, realistic naval vessel, no detached parts, clean topology',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for Zumwalt destroyer.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] Zumwalt destroyer package generated successfully.")
