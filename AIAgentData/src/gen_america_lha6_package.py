...
MODEL_NAME = "USS_America_LHA6"
MODEL_NAME_I18N = "LHA-6 美利坚号"
IMAGE_QUERY = "LHA-6 America amphibious assault ship 3d render side view"
...
AGENT_DESC = "美国美利坚级两栖攻击舰首舰，具备舰载机起降保障、两栖投送与远海综合作战能力。"
PREFERRED_IMAGE_URLS = [
    "https://p.turbosquid.com/ts-thumb/pj/rFUx4C/b6/amphibious_assault_ship_lha6_361/jpg/1743043485/1920x1080/turn_fit_q99/8d866628be451c4ab3e7652e5d4d872e7614ae6e/amphibious_assault_ship_lha6_361-1.jpg",
    "https://img.goodfon.com/original/2048x768/5/6c/amphibious-assault-ship-uss-america-lha-6-1.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("america", "lha-6", "amphibious assault ship", "carrier", "turbosquid", "render", "3d")
    ...
    def consider(url, strict_size=True):
        ...
        try:
            ...
            if "amphibious_assault_ship_lha6_361" in lower or "uss-america-lha-6" in lower:
                score += 300000
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid USS America LHA-6 ship thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for USS America LHA-6 ship.")
...
resp = server.create_rodin_job(
    text_prompt='USS America LHA-6 amphibious assault ship, full ship, complete connected hull, flat flight deck, island superstructure starboard side, realistic naval vessel, no detached parts, clean topology',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for USS America LHA-6 ship.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] USS America LHA-6 ship package generated successfully.")
