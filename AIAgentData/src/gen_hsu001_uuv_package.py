...
MODEL_NAME = "HSU_001_UUV"
MODEL_NAME_I18N = "HSU-001潜航器"
IMAGE_QUERY = "HSU-001 UUV 3d render side view"
...
AGENT_DESC = "HSU-001潜航器，具备自主航行、隐蔽侦察、目标探测与无人水下作战支援能力。"
PREFERRED_IMAGE_URLS = [
    "https://www.renderhub.com/finiask/hsu-001-submarine-drone/hsu-001-submarine-drone-02.jpg",
    "https://www.renderhub.com/finiask/hsu-001-submarine-drone/hsu-001-submarine-drone-01.jpg",
    "https://www.renderhub.com/finiask/hsu-001-submarine-drone/hsu-001-submarine-drone-07.jpg",
    "https://www.renderhub.com/finiask/hsu-001-submarine-drone/hsu-001-submarine-drone-05.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("hsu", "001", "uuv", "drone", "submarine", "render", "3d", "renderhub", "chinese")
    bad_kw = (
        "logo",
        "patch",
        "wallpaper",
    ...
    def consider(url, strict_size=True):
        ...
        try:
            ...
            if "hsu-001-submarine-drone-02.jpg" in lower:
                score += 950000
            elif "hsu-001-submarine-drone-01.jpg" in lower:
                score += 800000
            elif "hsu-001-submarine-drone-07.jpg" in lower:
                score += 650000
            elif "hsu-001-submarine-drone-05.jpg" in lower:
                score += 500000
            elif "hsu" in lower or "001" in lower or "uuv" in lower or "submarine" in lower:
                score += 300000
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid HSU-001 UUV thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for HSU-001 UUV.")
...
resp = server.create_rodin_job(
    text_prompt='HSU-001 unmanned underwater vehicle, complete connected unmanned submarine hull, integrated small sensor masts, clean side profile, modern autonomous underwater vehicle proportions, no detached parts, no floating geometry, clean topology',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for HSU-001 UUV.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] HSU-001 UUV package generated successfully.")
