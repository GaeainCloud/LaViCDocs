...
MODEL_NAME = "USS_Nimitz_CVN68"
MODEL_NAME_I18N = "CVN-68 尼米兹号"
IMAGE_QUERY = "CVN-68 Nimitz aircraft carrier 3d render side view"
...
AGENT_DESC = "美国尼米兹级核动力航空母舰首舰，具备舰载机起降保障、远洋持续部署与编队指挥能力。"
PREFERRED_IMAGE_URLS = [
    "https://www.renderhub.com/3d-horse/uss-nimitz-cvn-68/uss-nimitz-cvn-68-20.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("nimitz", "cvn-68", "carrier", "aircraft carrier", "renderhub", "render", "3dmodels")
    ...
    def consider(url, strict_size=True):
        ...
        try:
            ...
            if "uss-nimitz-cvn-68-20" in lower:
                score += 300000
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid USS Nimitz CVN-68 carrier thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for USS Nimitz CVN-68 carrier.")
...
resp = server.create_rodin_job(
    text_prompt='USS Nimitz CVN-68 aircraft carrier, full ship, complete connected hull, flat carrier flight deck, island superstructure starboard side, realistic naval vessel, no detached parts, clean topology',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for USS Nimitz CVN-68 carrier.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] USS Nimitz CVN-68 carrier package generated successfully.")
