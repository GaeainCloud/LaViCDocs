...
MODEL_NAME = "Type_055_Large_Destroyer"
MODEL_NAME_I18N = "055大驱"
IMAGE_QUERY = "Type 055 destroyer 3d render side view"
...
AGENT_DESC = "中国055型万吨级驱逐舰，具备区域防空、反导预警、反潜反舰与编队指挥作战能力。"
PREFERRED_IMAGE_URLS = [
    "https://p.turbosquid.com/ts-thumb/Wl/5p9Tio/KQ/055/png/1744779002/1920x1080/turn_fit_q99/0557d3be457663123752726bd10b4a2d509eefe9/055-1.jpg",
    "https://www.renderhub.com/pxfpxd/type-055-renhai-class-destroyer/type-055-renhai-class-destroyer-01.jpg",
    "https://img2.cgtrader.com/items/792828/c5ebb8a74e/type-055-class-destroyer-3d-model-obj-fbx-stl.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("type 055", "renhai", "destroyer", "surface combatant", "cgtrader", "turbosquid", "render")
    ...
    def consider(url, strict_size=True):
        ...
        try:
            ...
            if "055-1" in lower or "type-055-renhai-class-destroyer" in lower or "type-055-class-destroyer" in lower:
                score += 300000
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid Type 055 destroyer thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for Type 055 destroyer.")
...
def build_carrier_mesh():
    ...
    hull = trimesh.creation.box(extents=[9.4, 1.15, 1.0])
    hull.apply_translation([0.0, 0.0, 0.15])
    hull.visual.face_colors = hull_color
    parts.append(hull)

    bow = trimesh.creation.box(extents=[1.55, 1.0, 0.78])
    ...
    hangar = trimesh.creation.box(extents=[1.35, 0.74, 0.48])
    hangar.apply_translation([-3.05, 0.0, 0.98])
    hangar.visual.face_colors = hull_color
    parts.append(hangar)

    helo_deck = trimesh.creation.box(extents=[2.4, 1.05, 0.08])
...
resp = server.create_rodin_job(
    text_prompt='Type 055 destroyer, full ship, complete connected hull, stealthy large destroyer superstructure, realistic naval vessel, no detached parts, clean topology',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for Type 055 destroyer.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] Type 055 destroyer package generated successfully.")
