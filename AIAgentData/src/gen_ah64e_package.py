...
MODEL_NAME = "AH-64E_Apache_Guardian"
MODEL_NAME_I18N = "AH-64E直升机"
IMAGE_QUERY = "AH-64E Apache Guardian 3d render side view"
...
AGENT_DESC = "美制重型攻击直升机，具备对地火力打击、武装侦察与伴随支援能力。"
PREFERRED_IMAGE_URLS = [
    "https://static.turbosquid.com/Preview/001239/043/3J/3D-model-ah64e-apache-guardian_D.jpg",
    "https://preview.free3d.com/img/2017/10/2174801897335031365/gqgsgnv1.jpg",
    "https://img-new.cgtrader.com/items/4375453/8326c72b25/ah-64-apache-3d-model-8326c72b25.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    if not candidates:
        raise RuntimeError("No Bing image candidates found for AH-64E.")

    best = None
    good_kw = ("ah-64e", "apache", "guardian", "boeing", "attack", "helicopter")
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid real AH-64E thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for AH-64E.")
...
resp = server.create_rodin_job(
    text_prompt='AH-64E Apache Guardian attack helicopter, realistic hard-surface helicopter, tandem cockpit, chin turret, sensor mast, stub wings, main rotor and tail rotor, high quality military attack helicopter',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for AH-64E.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] AH-64E package generated successfully.")
