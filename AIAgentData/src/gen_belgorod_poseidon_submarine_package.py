...
MODEL_NAME = "Belgorod_Poseidon_Submarine"
MODEL_NAME_I18N = "波塞冬/别尔哥罗德号"
IMAGE_QUERY = "Belgorod Poseidon submarine 3d render side view"
...
AGENT_DESC = "别尔哥罗德号特种核潜艇，作为波塞冬无人潜航器搭载平台，具备远程隐蔽突防与特种任务投送能力。"
PREFERRED_IMAGE_URLS = [
    "https://p.turbosquid.com/ts-thumb/Lj/1KcB2X/Rs/b3/png/1682463054/1920x1080/fit_q87/7a298c247d68bc34036407643a5de8a4b1e977ef/b3.jpg",
    "https://p.turbosquid.com/ts-thumb/Lj/1KcB2X/wI/b4/png/1682463058/1920x1080/fit_q87/ac9abfd9a4dcdc7aaf7d809cff903b990c8d5d57/b4.jpg",
    "https://p.turbosquid.com/ts-thumb/Lj/1KcB2X/Gx/b5/png/1682463060/1920x1080/fit_q87/6607af0d82793859e1a5d1b2b6d1ede832565eb3/b5.jpg",
    "https://p.turbosquid.com/ts-thumb/Lj/1KcB2X/oG/z/png/1682372507/1920x1080/turn_fit_q99/59ecbf3e9c248c1c37f1b76affc2f1e5ea95b56c/z-1.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("belgorod", "poseidon", "submarine", "render", "3d", "turbosquid", "russia")
    bad_kw = (
        "logo",
        "patch",
        "wallpaper",
    ...
    def consider(url, strict_size=True):
        ...
        try:
            ...
            if "/z-1.jpg" in lower:
                score += 700000
            elif "/b3.jpg" in lower:
                score += 850000
            elif "/b4.jpg" in lower:
                score += 500000
            elif "/b5.jpg" in lower:
                score += 350000
            elif "belgorod" in lower or "poseidon" in lower or "submarine" in lower:
                score += 300000
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid Belgorod/Poseidon submarine thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for Belgorod/Poseidon submarine.")
...
resp = server.create_rodin_job(
    text_prompt='Belgorod special mission nuclear submarine, Poseidon carrier submarine, complete connected submarine hull, elongated pressure hull, integrated sail, realistic large special-mission submarine proportions, no detached parts, no floating geometry, clean topology',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for Belgorod/Poseidon submarine.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] Belgorod/Poseidon submarine package generated successfully.")
