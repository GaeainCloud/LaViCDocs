...
MODEL_NAME = "Y-9_Medium_Transport"
MODEL_NAME_I18N = "运-9运输机"
IMAGE_QUERY = "Shaanxi Y-9 transport aircraft 3d render side view"
...
AGENT_DESC = "中国中型战术运输机，具备空运投送、伞降支援与多用途改装能力。"
...
def fetch_real_thumbnail(output_png):
    ...
    if not candidates:
        raise RuntimeError("No Bing image candidates found for Y-9.")

    best = None
    good_kw = ("y-9", "y9", "shaanxi", "transport", "aircraft")
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid real Y-9 thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for Y-9.")
...
resp = server.create_rodin_job(
    text_prompt='Shaanxi Y-9 military transport aircraft, realistic hard-surface aircraft, full plane, clean topology, transport airplane',
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] Y-9 package generated successfully.")
