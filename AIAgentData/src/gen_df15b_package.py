...
MODEL_NAME = "DF_15B_Missile"
MODEL_NAME_I18N = "DF-15B导弹"
IMAGE_QUERY = "DF-15B ballistic missile side view render"
...
AGENT_DESC = "DF-15B近程弹道导弹模型，采用目标跟踪导引动力学用于对地精确打击仿真。"
ENABLE_QUALITY_GATE = False
PREFERRED_IMAGE_URLS = [
    "https://upload.wikimedia.org/wikipedia/commons/8/8f/Dongfeng-15B.JPG",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("df-15", "df15", "df15b", "dongfeng", "missile", "ballistic", "srbm")
    bad_kw = (
        "logo",
        "patch",
        "wallpaper",
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid DF-15B thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for DF-15B missile.")
...
resp = server.create_rodin_job(
    text_prompt='DF-15B short range ballistic missile, complete connected missile body, road mobile tactical ballistic missile, long cylindrical fuselage, pointed nose cone, four rear stabilizing fins, no launcher truck, no detached parts, no floating geometry, clean topology, clean side profile',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for DF-15B missile.")
...
def generate_agent_json(output_json):
    ...
    agent["agentKeyword"] = "df15b_missile"
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] DF-15B package generated successfully.")
