...
MODEL_NAME = "Type_052D_Destroyer"
MODEL_NAME_I18N = "052D型驱逐舰"
IMAGE_QUERY = "Type 052D destroyer 3d render side view"
...
AGENT_DESC = "中国052D型导弹驱逐舰，具备区域防空、反舰反潜与编队护航作战能力。"
PREFERRED_IMAGE_URLS = [
    "https://cdn.renderhub.com/mermodels/type-052d-nanjing-plan-destroyer/type-052d-nanjing-plan-destroyer-01.jpg",
    "https://cdn.renderhub.com/mermodels/type-052d-nanjing-plan-destroyer/type-052d-nanjing-plan-destroyer-04.jpg",
    "https://cdn.renderhub.com/pxfpxd/type-052d-frigate/type-052d-frigate-01.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("type 052d", "destroyer", "surface combatant", "cgtrader", "renderhub", "render")
    ...
    def consider(url, strict_size=True):
        ...
        try:
            ...
            if "type-052d-nanjing-plan-destroyer" in lower or "type-052d-frigate" in lower:
                score += 300000
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid Type 052D destroyer thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for Type 052D destroyer.")
...
def build_carrier_mesh():
    ...
    hull = trimesh.creation.box(extents=[8.9, 1.02, 0.92])
    hull.apply_translation([0.0, 0.0, 0.15])
    hull.visual.face_colors = hull_color
    parts.append(hull)

    bow = trimesh.creation.box(extents=[1.45, 0.9, 0.72])
    ...
    super_1 = trimesh.creation.box(extents=[1.95, 0.62, 0.64])
    super_1.apply_translation([-0.6, 0.0, 1.08])
    super_1.visual.face_colors = hull_color
    parts.append(super_1)

    super_2 = trimesh.creation.box(extents=[1.18, 0.52, 0.54])
...
resp = server.create_rodin_job(
    text_prompt='Type 052D destroyer, full ship, complete connected hull, stealthy destroyer superstructure, realistic naval vessel, no detached parts, clean topology',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for Type 052D destroyer.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] Type 052D destroyer package generated successfully.")
