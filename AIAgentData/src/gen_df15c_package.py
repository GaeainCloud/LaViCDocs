...
MODEL_NAME = "DF_15C_Missile"
MODEL_NAME_I18N = "DF-15C导弹"
IMAGE_QUERY = "DF-15C ballistic missile side view render"
...
AGENT_DESC = "DF-15C近程弹道导弹模型，采用目标跟踪导引动力学用于对地精确打击仿真。"
ENABLE_QUALITY_GATE = False
PREFERRED_IMAGE_URLS = [
    "https://www.mvrsimulation.com/sites/default/files/img/models/DF-15C.CN.grey.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("df-15", "df15", "df15c", "dongfeng", "missile", "ballistic", "srbm")
    bad_kw = (
        "logo",
        "patch",
        "wallpaper",
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid DF-15C thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for DF-15C missile.")
...
resp = server.create_rodin_job(
    text_prompt='DF-15C short range ballistic missile, complete connected missile body, road mobile tactical ballistic missile, long cylindrical fuselage, pointed nose cone, four rear stabilizing fins, no launcher truck, no detached parts, no floating geometry, clean topology, clean side profile',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for DF-15C missile.")
...
def generate_agent_json(output_json):
    ...
    agent["agentKeyword"] = "df15c_missile"
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] DF-15C package generated successfully.")
