...
MODEL_NAME = "Il-76MD-90A_Transport"
MODEL_NAME_I18N = "伊尔-76MD-90A运输机"
IMAGE_QUERY = "Ilyushin Il-76MD-90A 3d render side view"
...
AGENT_DESC = "俄罗斯大型战略运输机，具备远程空运投送、重装运输与多用途改装能力。"
...
def fetch_real_thumbnail(output_png):
    ...
    if not candidates:
        raise RuntimeError("No Bing image candidates found for Il-76MD-90A.")

    best = None
    good_kw = ("il-76", "il76", "il-76md-90a", "ilyushin", "transport", "aircraft")
    render_kw = ("render", "behance", "turbosquid", "tinkercad", "png")
    bad_kw = (
        "logo", "patch", "wallpaper", "diagram", "blueprint",
        "airliners.net", "planespotters", "abpic", "alamy",
    )
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid real Il-76MD-90A thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for Il-76MD-90A.")
...
resp = server.create_rodin_job(
    text_prompt='Ilyushin Il-76MD-90A military transport aircraft, realistic hard-surface aircraft, full plane, high quality large cargo jet airplane',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for Il-76MD-90A.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] Il-76MD-90A package generated successfully.")
