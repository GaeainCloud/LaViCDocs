...
MODEL_NAME = "Switchblade300_Loitering_Munition"
MODEL_NAME_I18N = "Switchblade 300巡飞弹"
IMAGE_QUERY = "Switchblade 300 loitering munition side view"
...
AGENT_DESC = "Switchblade 300便携式巡飞弹，具备短时滞空侦察与对轻装目标快速打击能力。"
PREFERRED_IMAGE_URLS = [
    "https://www.avinc.com/images/uploads/media_assets/2739/sb300b20_product_image.jpeg",
    "https://www.defensedaily.com/wp-content/uploads/2024/04/switchblade300-block20.png",
    "https://www.avinc.com/images/uploads/news/3291/sb-press-image-2025_1500x900_20251007__banner.png",
]
...
def fetch_real_thumbnail(output_png: str) -> None:
    ...
    def prepare_img(url: str, content: bytes):
        ...
        if max(w, h) < 800:
            return None
        lower = url.lower()
        if "sb300b20_product_image.jpeg" in lower:
            # Official product render: crop tighter so only the munition body remains.
            img = img.crop((120, 190, 2280, 1190))
            w, h = img.size
    ...
    good_kw = ("switchblade", "aerovironment", "loitering", "munition", "drone")
    bad_kw = ("logo", "icon", "patch", "poster", "wallpaper", "drawing", "launcher", "tube", "soldier", "troop")
    best = None
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid Switchblade 300 thumbnail.")
...
def generate_blue_military_symbol(output_png: str) -> None:
    # Draw an APP-6 style friendly frame (blue) with a loitering-munition glyph.
    size = 160
    ...
    # Body (left -> right)
    draw.line([(42, 80), (118, 80)], fill=black, width=4)
    # Nose
    draw.polygon([(118, 80), (105, 73), (105, 87)], fill=black)
    # Mid wing pair
    draw.line([(72, 80), (60, 58)], fill=black, width=3)
    draw.line([(72, 80), (60, 102)], fill=black, width=3)
    # Rear wing pair
    draw.line([(52, 80), (42, 63)], fill=black, width=3)
    draw.line([(52, 80), (42, 97)], fill=black, width=3)
    # Prop/engine hint
    draw.arc([36, 72, 50, 88], 0, 360, fill=black, width=2)
...
def build_switchblade_mesh() -> trimesh.Trimesh:
    ...
    light_gray = [214, 214, 214, 255]
    dark_gray = [182, 182, 182, 255]

    parts = []

    # Fuselage (X-forward)
    fuselage = trimesh.creation.cylinder(radius=0.03, height=0.52, sections=36)
    fuselage.apply_transform(trimesh.transformations.rotation_matrix(np.pi / 2.0, [0, 1, 0]))
    fuselage.visual.face_colors = white
    parts.append(fuselage)

    # Nose cone
    nose = trimesh.creation.cone(radius=0.03, height=0.08, sections=36)
    nose.apply_transform(trimesh.transformations.rotation_matrix(np.pi / 2.0, [0, 1, 0]))
    nose.apply_translation([0.30, 0.0, 0.0])
    nose.visual.face_colors = light_gray
    parts.append(nose)

    # Rear shroud
    rear = trimesh.creation.cylinder(radius=0.028, height=0.05, sections=24)
    rear.apply_transform(trimesh.transformations.rotation_matrix(np.pi / 2.0, [0, 1, 0]))
    rear.apply_translation([-0.27, 0.0, 0.0])
    rear.visual.face_colors = dark_gray
    parts.append(rear)

    # Main crossed wings
    wing_a = trimesh.creation.box(extents=[0.012, 0.30, 0.03])
    wing_a.apply_translation([-0.03, 0.0, 0.0])
    wing_a.visual.face_colors = white
    parts.append(wing_a)

    wing_b = trimesh.creation.box(extents=[0.012, 0.03, 0.30])
    wing_b.apply_translation([-0.03, 0.0, 0.0])
    wing_b.visual.face_colors = white
    parts.append(wing_b)

    # Rear crossed fins
    fin_a = trimesh.creation.box(extents=[0.010, 0.20, 0.024])
    fin_a.apply_translation([-0.20, 0.0, 0.0])
    fin_a.visual.face_colors = light_gray
    parts.append(fin_a)

    fin_b = trimesh.creation.box(extents=[0.010, 0.024, 0.20])
    fin_b.apply_translation([-0.20, 0.0, 0.0])
    fin_b.visual.face_colors = light_gray
    parts.append(fin_b)
...
def normalize_loiter_pose_glb(glb_path: str) -> None:
    ...
    # Loiter munition should lie flat with wing span on Y, not stand on Z.
...
resp = server.create_rodin_job(
    text_prompt='Switchblade 300 loitering munition, single complete aircraft, compact tube-launched loitering munition, cylindrical fuselage, pointed nose, crossed main wings, rear crossed tail fins, pusher propeller at rear, no launcher, no detached parts, no floating geometry, clean topology, clean side profile',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for Switchblade 300.")
...
def generate_agent_json(output_json: str) -> None:
    ...
    agent["agentKeyword"] = "switchblade300"
...
def main() -> None:
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] Switchblade300 package generated successfully.")
