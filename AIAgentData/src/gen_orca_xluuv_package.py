...
MODEL_NAME = "Orca_XLUUV"
MODEL_NAME_I18N = "虎鲸无人潜航器"
IMAGE_QUERY = "Orca XLUUV 3d render side view"
...
AGENT_DESC = "虎鲸无人潜航器，具备长航时自主航行、隐蔽侦察、布放载荷与远程无人水下作战能力。"
PREFERRED_IMAGE_URLS = [
    "https://p.turbosquid.com/ts-thumb/Ap/fiU6Xm/br/02/png/1668894949/1920x1080/fit_q87/7dc7d06364f1be3a50d178ffb8302d6b672ec18f/02.jpg",
    "https://p.turbosquid.com/ts-thumb/Ap/fiU6Xm/LI/b1/png/1668894973/1920x1080/fit_q87/e3302dc04f7968348bce1591f8cbbbf5b780e1f5/b1.jpg",
    "https://p.turbosquid.com/ts-thumb/Ap/fiU6Xm/o9/b3/png/1668894977/1920x1080/fit_q87/338acf34e11867ed89dd9f0a3811e8d55bbcf1a1/b3.jpg",
    "https://p.turbosquid.com/ts-thumb/Ap/fiU6Xm/EZ/b0/png/1668894974/1920x1080/fit_q87/7aebf763a3c204939b432bbcf6eddd1e349ca6d1/b0.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("orca", "xluuv", "uuv", "submarine", "render", "3d", "turbosquid", "boeing")
    bad_kw = (
        "logo",
        "patch",
        "wallpaper",
    ...
    def consider(url, strict_size=True):
        ...
        try:
            ...
            if "/02.jpg" in lower:
                score += 900000
            elif "/b1.jpg" in lower:
                score += 750000
            elif "/b3.jpg" in lower:
                score += 650000
            elif "/b0.jpg" in lower:
                score += 550000
            elif "orca" in lower or "xluuv" in lower or "uuv" in lower or "submarine" in lower:
                score += 300000
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid Orca XLUUV thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for Orca XLUUV.")
...
resp = server.create_rodin_job(
    text_prompt='Orca XLUUV unmanned undersea vehicle, complete connected extra-large unmanned submarine hull, integrated low sail, clean side profile, modern autonomous underwater vehicle proportions, no detached parts, no floating geometry, clean topology',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for Orca XLUUV.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] Orca XLUUV package generated successfully.")
