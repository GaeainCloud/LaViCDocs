...
MODEL_NAME = "GBU_39_SDB"
MODEL_NAME_I18N = "GBU-39SDB炸弹"
IMAGE_QUERY = "GBU-39 SDB bomb side view render"
...
AGENT_DESC = "GBU-39 SDB小直径制导炸弹模型，采用目标跟踪导引动力学用于对地精确打击仿真。"
ENABLE_QUALITY_GATE = False
PREFERRED_IMAGE_URLS = [
    "https://p.turbosquid.com/ts-thumb/EV/60TSpU/GKxQmVMJ/small_diameter_bomb_01/jpg/1345111822/1920x1080/fit_q87/6ed73b1817d79e0a8555af60bf8c859cbdc309e4/small_diameter_bomb_01.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("gbu-39", "gbu39", "sdb", "small diameter bomb", "bomb")
    bad_kw = (
        "logo",
        "patch",
        "wallpaper",
    ...
    def consider(url, strict_size=True):
        ...
        try:
            ...
            if "small_diameter_bomb_01.jpg" in lower or "gbu-39" in lower or "small-diameter-bomb" in lower or "small diameter bomb" in lower:
                score += 1000000
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid GBU-39 SDB thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        svg = military_symbol.get_symbol_svg_string_from_name(
            "Hostile Surface-To-Surface Missile", style="light", bounding_padding=4, use_variants=True
        )
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for GBU-39 SDB missile.")
...
def apply_manual_pose_fix_glb(glb_path, x_deg=0.0, y_deg=0.0, z_deg=0.0):
    scene = trimesh.load(glb_path, force="scene")
    if x_deg:
        scene.apply_transform(
            trimesh.transformations.rotation_matrix(np.deg2rad(x_deg), [1, 0, 0])
        )
    if y_deg:
        scene.apply_transform(
            trimesh.transformations.rotation_matrix(np.deg2rad(y_deg), [0, 1, 0])
        )
    if z_deg:
        scene.apply_transform(
            trimesh.transformations.rotation_matrix(np.deg2rad(z_deg), [0, 0, 1])
        )
    ...
    extents = scene.bounds[1] - scene.bounds[0]
    print(
        f"[glb] manual pose fix x={x_deg}, y={y_deg}, z={z_deg}, "
        f"extents={extents.tolist()} -> {glb_path}"
    )
...
def missile_quality_needs_fallback(glb_path):
    ...
    # A ballistic missile should stay slender after normalization: X >> Y,Z.
...
resp = server.create_rodin_job(
    text_prompt='GBU-39 SDB small diameter bomb, single complete guided bomb, complete connected bomb body, slender cylindrical bomb with ogive nose, small mid-body fins and rear tail fins, no launcher, no rack, no detached parts, no floating geometry, clean topology, clean side profile',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for GBU-39 SDB missile.")
...
def main():
    ...
    # GBU-39 currently tends to come out standing upright and nose-backward after Rodin.
    apply_manual_pose_fix_glb(glb_path, x_deg=90, z_deg=180)
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] GBU-39 SDB package generated successfully.")
