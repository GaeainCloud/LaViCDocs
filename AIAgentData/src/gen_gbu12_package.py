...
MODEL_NAME = "GBU_12_Paveway_II"
MODEL_NAME_I18N = "GBU-12炸弹"
IMAGE_QUERY = "GBU-12 Paveway II bomb side view render"
...
AGENT_DESC = "GBU-12 Paveway II制导炸弹模型，采用目标跟踪导引动力学用于对地精确打击仿真。"
ENABLE_QUALITY_GATE = False
PREFERRED_IMAGE_URLS = [
    "https://p.turbosquid.com/ts-thumb/n9/jvT6rN/U1lIoHMj/gbu12_01/jpg/1458998233/1920x1080/fit_q87/8dfca5b77f102f214a1f90e761a1f8cefd102fca/gbu12_01.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("gbu-12", "gbu12", "paveway", "guided bomb", "bomb")
    bad_kw = (
        "logo",
        "patch",
        "wallpaper",
    ...
    def consider(url, strict_size=True):
        ...
        try:
            ...
            if "gbu12_01.jpg" in lower or "gbu-12" in lower or "gbu12" in lower or "paveway" in lower:
                score += 1000000
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid GBU-12 thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for GBU-12 bomb.")
...
resp = server.create_rodin_job(
    text_prompt='GBU-12 Paveway II guided bomb, single complete guided bomb, complete connected bomb body, cylindrical bomb with pointed nose, front guidance canards, rear tail fins, no launcher, no rack, no detached parts, no floating geometry, clean topology, clean side profile',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for GBU-12 missile.")
...
def generate_agent_json(output_json):
    ...
    agent["agentKeyword"] = "gbu12_paveway_ii"
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] GBU-12 package generated successfully.")
