...
MODEL_NAME = "Arleigh_Burke_Class_Destroyer"
MODEL_NAME_I18N = "阿利·伯克级驱逐舰"
IMAGE_QUERY = "Arleigh Burke class destroyer 3d render side view"
...
AGENT_DESC = "美国阿利·伯克级导弹驱逐舰，具备防空反导、反潜反舰与编队护航作战能力。"
PREFERRED_IMAGE_URLS = [
    "https://img-new.cgtrader.com/items/5114640/4501dfcae9/arleigh-burke-class-destroyer-burke-class-3d-model-4501dfcae9.jpg",
    "https://static.turbosquid.com/Preview/001170/411/F1/3D-model-arleigh-burke-class-destroyer_Z.jpg",
    "https://img-new.cgtrader.com/items/3213237/1217dd4e33/arleigh-burke-class-destroyer-3d-model-rigged-obj-fbx-c4d-stl-blend.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("arleigh burke", "destroyer", "ddg", "surface combatant", "cgtrader", "render", "3dmodels")
    ...
    def consider(url, strict_size=True):
        ...
        try:
            ...
            if "arleigh-burke-class-destroyer" in lower or "destroyer_z" in lower:
                score += 300000
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid Arleigh Burke destroyer thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for Arleigh Burke destroyer.")
...
def build_carrier_mesh():
    ...
    hull = trimesh.creation.box(extents=[8.8, 1.0, 0.95])
    hull.apply_translation([0.0, 0.0, 0.15])
    hull.visual.face_colors = hull_color
    parts.append(hull)

    bow = trimesh.creation.box(extents=[1.4, 0.9, 0.75])
    ...
    hangar = trimesh.creation.box(extents=[1.0, 0.62, 0.42])
    hangar.apply_translation([-3.05, 0.0, 0.98])
    hangar.visual.face_colors = hull_color
    parts.append(hangar)

    helo_deck = trimesh.creation.box(extents=[2.0, 0.95, 0.08])
    ...
    gun.apply_translation([3.3, 0.0, 0.86])
...
resp = server.create_rodin_job(
    text_prompt='Arleigh Burke class destroyer, full ship, complete connected hull, stealthy destroyer superstructure, realistic naval vessel, no detached parts, clean topology',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for Arleigh Burke destroyer.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] Arleigh Burke destroyer package generated successfully.")
