...
MODEL_NAME = "HMS_Queen_Elizabeth_R08"
MODEL_NAME_I18N = "R08 伊丽莎白女王号"
IMAGE_QUERY = "R08 HMS Queen Elizabeth aircraft carrier 3d render side view"
...
AGENT_DESC = "英国伊丽莎白女王级航空母舰首舰，具备舰载机起降保障、远洋持续部署与编队指挥能力。"
POST_ROLL_OVERRIDE_X_DEG = 90
PREFERRED_IMAGE_URLS = [
    "https://www.renderhub.com/3d-horse/hms-queen-elizabeth-aircraft-carrier/hms-queen-elizabeth-aircraft-carrier-22.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("queen elizabeth", "r08", "carrier", "aircraft carrier", "renderhub", "render", "3dmodels")
    ...
    def consider(url, strict_size=True):
        ...
        try:
            ...
            if "hms-queen-elizabeth-aircraft-carrier-22" in lower:
                score += 300000
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid HMS Queen Elizabeth R08 carrier thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for HMS Queen Elizabeth R08 carrier.")
...
resp = server.create_rodin_job(
    text_prompt='HMS Queen Elizabeth R08 aircraft carrier, full ship, complete connected hull, ski-jump carrier flight deck, twin island superstructure starboard side, realistic naval vessel, no detached parts, clean topology',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for HMS Queen Elizabeth R08 carrier.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] HMS Queen Elizabeth R08 carrier package generated successfully.")
