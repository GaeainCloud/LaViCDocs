...
MODEL_NAME = "J-16D_Roaring_Wolf"
MODEL_NAME_I18N = "J-16D咆哮狼"
IMAGE_QUERY = "Shenyang J-16D electronic fighter 3d render side view"
...
AGENT_DESC = "中制电子战飞机，具备电子压制、伴随护航、干扰压制与战场电磁支援能力。"
PREFERRED_IMAGE_URLS = [
    "https://img-new.cgtrader.com/items/4848723/f36f8cfc8c/large/pbr-shenyang-j-16d-electronic-fighter-china-plaaf-3d-model-f36f8cfc8c.jpg",
    "https://img-new.cgtrader.com/items/4848723/45425d44d9/large/pbr-shenyang-j-16d-electronic-fighter-china-plaaf-3d-model-45425d44d9.jpg",
    "https://img-new.cgtrader.com/items/4848723/df9b14e544/pbr-shenyang-j-16d-electronic-fighter-china-plaaf-3d-model-df9b14e544.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("j-16d", "j16d", "shenyang", "electronic", "fighter", "cgtrader", "plaaf")
    ...
    if not candidates:
        raise RuntimeError("No Bing image candidates found for J-16D.")

    for url in candidates[:80]:
        consider(url)

    if best is None:
        raise RuntimeError("Failed to download a valid real J-16D thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for J-16D.")
...
resp = server.create_rodin_job(
    text_prompt='Shenyang J-16D electronic warfare aircraft, realistic hard-surface twin-engine fighter jet, full plane, wingtip electronic warfare pods, high quality military aircraft',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for J-16D.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] J-16D package generated successfully.")
