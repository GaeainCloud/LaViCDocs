...
MODEL_NAME = "Mogami_30FFM_Frigate"
MODEL_NAME_I18N = "日30FFM护卫舰"
IMAGE_QUERY = "30FFM frigate 3d render side view"
...
AGENT_DESC = "日本30FFM多用途护卫舰，具备防空、反潜、反舰与编队护航作战能力。"
PREFERRED_IMAGE_URLS = [
    "https://upload.wikimedia.org/wikipedia/commons/6/6f/Conceptual_image_of_30FFM_by_MHI.png",
    "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6f/Conceptual_image_of_30FFM_by_MHI.png/985px-Conceptual_image_of_30FFM_by_MHI.png",
    "https://p.turbosquid.com/ts-thumb/Mj/zX9t7v/1x/stemcell_unity_render/png/1713539700/1920x1080/fit_q87/fceca4f6d6c83fc13103fb26e8dfca0bf9ee16c8/stemcell_unity_render.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("30ffm", "mogami", "frigate", "surface combatant", "render", "3d", "mhi", "wikimedia")
    bad_kw = (
        "logo",
        "patch",
        "wallpaper",
    ...
    def consider(url, strict_size=True):
        ...
        try:
            ...
            if "conceptual_image_of_30ffm_by_mhi" in lower:
                score += 600000
            elif "stemcell_unity_render.jpg" in lower:
                score += 450000
            elif "mogami-class-frigate-30ffm-3d-model-824f6fac74.jpg" in lower:
                score += 500000
            elif "mogami-class-frigate-30ffm-3d-model-2449d57caa.jpg" in lower or "mogami-class-frigate-30ffm-3d-model-4c632c28a5.jpg" in lower:
                score += 350000
            elif "30ffm" in lower or "mogami" in lower or "frigate" in lower:
                score += 300000
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid 30FFM frigate thumbnail image.")
...
def generate_military_symbol(output_png):
    svg = military_symbol.get_symbol_svg_string_from_name(
        "Friendly Frigate", style="light", bounding_padding=4, use_variants=True
    )
    if svg is None:
        svg = military_symbol.get_symbol_svg_string_from_name(
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for 30FFM frigate.")
...
def build_carrier_mesh():
    ...
    hull = trimesh.creation.box(extents=[8.2, 0.92, 0.84])
    hull.apply_translation([0.0, 0.0, 0.15])
    hull.visual.face_colors = hull_color
    parts.append(hull)

    bow = trimesh.creation.box(extents=[1.25, 0.8, 0.66])
    ...
    hangar = trimesh.creation.box(extents=[1.15, 0.62, 0.42])
    hangar.apply_translation([-3.05, 0.0, 0.98])
    hangar.visual.face_colors = hull_color
    parts.append(hangar)

    helo_deck = trimesh.creation.box(extents=[2.0, 0.86, 0.08])
...
resp = server.create_rodin_job(
    text_prompt='Constellation class frigate, full ship, complete connected hull, modern frigate superstructure, realistic naval vessel, no detached parts, clean topology',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for Constellation frigate.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] 30FFM frigate package generated successfully.")
