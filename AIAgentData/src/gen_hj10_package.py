...
MODEL_NAME = "HJ_10_Missile"
MODEL_NAME_I18N = "HJ-10导弹"
IMAGE_QUERY = "HJ-10 missile side view render"
...
AGENT_DESC = "HJ-10导弹模型，采用目标跟踪导引动力学用于精确打击仿真。"
ENABLE_QUALITY_GATE = False
PREFERRED_IMAGE_URLS = [
    "https://www.mvrsimulation.com/sites/default/files/img/models/AFT-10.CN.grey.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("df-15", "df15", "hj10", "hj10", "missile", "ballistic", "srbm")
    bad_kw = (
        "logo",
        "patch",
        "wallpaper",
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid HJ-10 thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for HJ-10 missile.")
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for HJ-10 missile.")
...
def generate_agent_json(output_json):
    ...
    agent["agentKeyword"] = "hj10_missile"
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] HJ-10 package generated successfully.")
