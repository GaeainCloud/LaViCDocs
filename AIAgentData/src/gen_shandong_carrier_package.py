...
MODEL_NAME = "Shandong_Aircraft_Carrier"
MODEL_NAME_I18N = "山东舰"
IMAGE_QUERY = "Shandong aircraft carrier 3d render side view"
...
AGENT_DESC = "中国首艘国产滑跃起飞型航空母舰，具备舰载机起降保障、编队指挥与远海综合作战能力。"
POST_ROLL_OVERRIDE_X_DEG = 90
PREFERRED_IMAGE_URLS = [
    "https://defensefeeds.com/wp-content/uploads/2025/07/type-002-aircraft-carrier-1024x682.webp",
    "https://gw.alicdn.com/imgextra/i3/2201221386804/O1CN01pp8Bu9208HX1GTydA_!!2201221386804.jpg",
    "https://makerworld.bblmw.com/makerworld/model/US18406330f9f8a3/design/2024-10-29_7a2aa351594d3.png?x-oss-process=image/resize,w_1000/format,webp",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("shandong", "type-002", "type002", "carrier", "aircraft carrier", "render", "makerworld")
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid Shandong carrier thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for Shandong carrier.")
...
def build_carrier_mesh():
    ...
    hull = trimesh.creation.box(extents=[8.45, 1.48, 1.10])
    hull.apply_translation([0.0, 0.0, 0.33])
    hull.visual.face_colors = hull_color
    parts.append(hull)

    bow = trimesh.creation.box(extents=[1.18, 1.18, 0.92])
    bow.apply_transform(trimesh.transformations.rotation_matrix(np.deg2rad(35), [0, 0, 1]))
    bow.apply_translation([4.28, 0.0, 0.30])
    bow.visual.face_colors = hull_color
    parts.append(bow)

    stern = trimesh.creation.box(extents=[0.86, 1.32, 0.82])
    stern.apply_translation([-4.38, 0.0, 0.28])
    stern.visual.face_colors = hull_color
    parts.append(stern)

    deck = trimesh.creation.box(extents=[8.9, 2.02, 0.12])
    deck.apply_translation([0.02, 0.12, 0.98])
    deck.visual.face_colors = deck_color
    parts.append(deck)

    ski_jump = trimesh.creation.box(extents=[1.62, 2.02, 0.12])
    ski_jump.apply_transform(trimesh.transformations.rotation_matrix(np.deg2rad(-16), [0, 1, 0]))
    ski_jump.apply_translation([3.68, 0.12, 1.20])
    ski_jump.visual.face_colors = deck_color
    parts.append(ski_jump)

    angled = trimesh.creation.box(extents=[3.1, 0.92, 0.10])
    angled.apply_transform(trimesh.transformations.rotation_matrix(np.deg2rad(8), [0, 0, 1]))
    angled.apply_translation([-0.05, 0.90, 1.02])
    angled.visual.face_colors = deck_color
    parts.append(angled)

    island_base = trimesh.creation.box(extents=[0.98, 0.42, 0.72])
    island_base.apply_translation([-1.10, -0.48, 1.33])
    island_base.visual.face_colors = hull_color
    parts.append(island_base)

    island_top = trimesh.creation.box(extents=[0.58, 0.26, 0.40])
    island_top.apply_translation([-0.88, -0.47, 1.85])
    island_top.visual.face_colors = hull_color
    parts.append(island_top)

    mast = trimesh.creation.box(extents=[0.14, 0.12, 0.70])
    mast.apply_translation([-0.74, -0.46, 2.35])
...
resp = server.create_rodin_job(
    text_prompt='Shandong aircraft carrier, full ship, complete connected hull, ski-jump flight deck, island superstructure starboard side, realistic naval vessel, no detached parts, clean topology',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for Shandong carrier.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] Shandong carrier package generated successfully.")
