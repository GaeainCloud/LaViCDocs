...
MODEL_NAME = "EC-130H_Compass_Call"
MODEL_NAME_I18N = "EC-130H电子战飞机"
IMAGE_QUERY = "Lockheed EC-130H Compass Call render side view"
...
AGENT_DESC = "美制电子战飞机，基于C-130平台改装，具备电子侦察、通信干扰、电磁压制与战场支援能力。"
PREFERRED_IMAGE_URLS = [
    "https://www.renderhub.com/chipbasschaos/lockheed-c-130h-hercules/lockheed-c-130h-hercules-14.jpg",
    "https://www.renderhub.com/chipbasschaos/lockheed-c-130h-hercules/lockheed-c-130h-hercules-19.jpg",
    "https://p.turbosquid.com/ts-thumb/Mp/HonG5Q/KgmNTYO1/ec130h_cc_12tit/jpg/1164310305/600x600/fit_q87/6d30c888160dc13e66cf119073d69f1e4650f43b/ec130h_cc_12tit.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    good_kw = ("ec-130h", "ec130h", "compass call", "c-130", "lockheed", "renderhub", "aircraft")
    ...
    if not candidates:
        raise RuntimeError("No Bing image candidates found for EC-130H.")

    for url in candidates[:80]:
        consider(url)

    if best is None:
        raise RuntimeError("Failed to download a valid real EC-130H thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for EC-130H.")
...
resp = server.create_rodin_job(
    text_prompt='Lockheed EC-130H Compass Call electronic warfare aircraft, realistic hard-surface military aircraft, full plane, four turboprop engines, communication jamming antennas, electronic warfare mission aircraft',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for EC-130H.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] EC-130H package generated successfully.")
