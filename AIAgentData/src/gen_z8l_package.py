...
MODEL_NAME = "Z-8L_Helicopter"
MODEL_NAME_I18N = "直-8L直升机"
IMAGE_QUERY = "Harbin Z-8L helicopter 3d render side view"
...
AGENT_DESC = "中国大型通用运输直升机，具备人员输送、物资投送与多用途支援能力。"
PREFERRED_IMAGE_URLS = [
    "https://img-new.cgtrader.com/items/4840382/1763d2785a/pbr-chinese-z-8l-helicopter-with-wide-body-z8l-3d-model-1763d2785a.jpg",
    "https://img-new.cgtrader.com/items/4840382/d035e6c863/pbr-chinese-z-8l-helicopter-with-wide-body-z8l-3d-model-d035e6c863.jpg",
    "https://img-new.cgtrader.com/items/4840382/4309a75a64/pbr-chinese-z-8l-helicopter-with-wide-body-z8l-3d-model-4309a75a64.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    if not candidates:
        raise RuntimeError("No Bing image candidates found for Z-8L.")

    best = None
    good_kw = ("z-8", "z8", "z-8l", "helicopter", "harbin", "transport")
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid real Z-8L thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for Z-8L.")
...
resp = server.create_rodin_job(
    text_prompt='Harbin Z-8L military transport helicopter, realistic hard-surface helicopter, full aircraft, main rotor and tail rotor, high quality utility helicopter',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for Z-8L.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] Z-8L package generated successfully.")
