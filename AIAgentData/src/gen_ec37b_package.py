...
MODEL_NAME = "EC-37B_Compass_Call"
MODEL_NAME_I18N = "EC-37B电子战飞机"
IMAGE_QUERY = "EC-37B Compass Call render side view"
...
AGENT_DESC = "美制电子战飞机，基于Gulfstream G550平台改装，具备电子侦察、通信干扰、电磁压制与远程战场支援能力。"
PREFERRED_IMAGE_URLS = [
    "https://theaviationist.com/wp-content/uploads/2023/11/EA-37B_Electronic_Attack_2.jpg",
    "https://theaviationist.com/wp-content/uploads/2023/09/EC-37B_Compass_Call_Delivered_1.jpeg",
    "https://backend.ainonline.com/sites/default/files/styles/fpsc_885x500/public/uploads/2023/05/ec-37b.jpeg?h=c8e955ed&itok=OPdvDjnu",
]
...
def fetch_real_thumbnail(output_png):
    ...
    good_kw = ("ec-37b", "ea-37b", "compass call", "g550", "gulfstream", "aircraft")
    bad_kw = (
        "logo",
        "patch",
        "wallpaper",
    ...
    if not candidates:
        raise RuntimeError("No Bing image candidates found for EC-37B.")

    for url in candidates[:80]:
        consider(url)

    if best is None:
        raise RuntimeError("Failed to download a valid real EC-37B thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for EC-37B.")
...
def build_transport_mesh():
    # Fallback only when BlenderMCP is unavailable or the generated mesh is incomplete.
    parts = []
    body_color = [205, 205, 205, 255]
    dark = [70, 70, 70, 255]

    fuselage = trimesh.creation.capsule(radius=0.22, height=6.8, count=[28, 28])
    ...
    nose = trimesh.creation.cone(radius=0.20, height=0.8, sections=28)
    nose.apply_transform(trimesh.transformations.rotation_matrix(-np.pi / 2, [0, 1, 0]))
    nose.apply_translation([3.55, 0, 0.0])
    nose.visual.face_colors = body_color
    parts.append(nose)

    wing = trimesh.creation.box(extents=[1.5, 5.2, 0.10])
    wing.apply_transform(trimesh.transformations.rotation_matrix(np.deg2rad(-12), [0, 0, 1]))
    wing.apply_translation([0.15, 0, 0.03])
    wing.visual.face_colors = body_color
    parts.append(wing)

    h_tail = trimesh.creation.box(extents=[0.55, 2.0, 0.08])
    h_tail.apply_translation([-3.1, 0, 0.55])
    h_tail.visual.face_colors = body_color
    parts.append(h_tail)

    v_tail = trimesh.creation.box(extents=[0.55, 0.10, 1.05])
    v_tail.apply_translation([-3.25, 0, 1.0])
    v_tail.visual.face_colors = body_color
    parts.append(v_tail)

    for side in [-1, 1]:
        eng = trimesh.creation.cylinder(radius=0.18, height=1.0, sections=24)
        eng.apply_transform(trimesh.transformations.rotation_matrix(np.pi / 2, [0, 1, 0]))
        eng.apply_translation([-1.95, side * 0.62, 0.10])
        eng.visual.face_colors = dark
        parts.append(eng)

    belly_fairing = trimesh.creation.box(extents=[1.2, 0.35, 0.14])
    belly_fairing.apply_translation([0.0, 0.0, -0.16])
    belly_fairing.visual.face_colors = dark
    parts.append(belly_fairing)
...
def export_fallback_glb(output_glb):
    ...
    # The fallback EW business-jet mesh is authored directly in the LaViC aircraft canonical pose.
...
resp = server.create_rodin_job(
    text_prompt='EC-37B Compass Call electronic warfare aircraft, realistic hard-surface military business jet, full plane, Gulfstream G550 based airframe, twin engine, single fused airframe, connected wings and tail, no detached parts, clean aircraft topology, communication jamming antennas',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for EC-37B.")
...
def main():
    ...
    print(
        f"[glb] pruned loose parts: kept {cleanup['kept_faces']} / "
        f"{cleanup['total_faces']} faces across {cleanup['cluster_count']} clusters"
    )
    quality = inspect_aircraft_mesh_quality(glb_path)
    print(
        f"[glb] quality faces={quality['faces']} length={quality['length']:.3f} "
        f"span={quality['span']:.3f} span_ratio={quality['span_ratio']:.3f}"
    )
    kept_ratio = (
        cleanup["kept_faces"] / cleanup["total_faces"]
        if cleanup["total_faces"] > 0
        else 0.0
    )
    if kept_ratio < 0.45 or quality["span_ratio"] < 0.55:
        print(
            f"[glb] generated aircraft deemed incomplete "
            f"(kept_ratio={kept_ratio:.3f}, span_ratio={quality['span_ratio']:.3f}); "
            "replacing with complete fallback EW aircraft mesh"
        )
        export_fallback_glb(glb_path)
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] EC-37B package generated successfully.")
