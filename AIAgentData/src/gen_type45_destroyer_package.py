...
MODEL_NAME = "Type_45_Destroyer"
MODEL_NAME_I18N = "45型驱逐舰"
IMAGE_QUERY = "Type 45 destroyer 3d render side view"
...
AGENT_DESC = "英国45型驱逐舰，具备区域防空、反导预警与编队护航作战能力。"
PREFERRED_IMAGE_URLS = [
    "https://static.turbosquid.com/Preview/2014/05/19__22_23_51/Type_45_Destroyer_00.jpg21bb3f5b-dad7-4f9b-a7da-df6dfdcd18d0Original.jpg",
    "https://p.turbosquid.com/ts-thumb/TM/wVPFZl/FXvw8FZQ/t45_view4/jpg/1162156918/1920x1080/fit_q87/e774d964575971ad4781e7dddc858a74c6f5ceb5/t45_view4.jpg",
    "https://img-new.cgtrader.com/items/4121665/3f3df324dd/type-45-destroyer-low-poly-3d-model-3f3df324dd.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("type 45", "daring", "destroyer", "surface combatant", "turbosquid", "cgtrader", "render")
    ...
    def consider(url, strict_size=True):
        ...
        try:
            ...
            if "type_45_destroyer" in lower or "t45_view4" in lower or "type-45-destroyer" in lower:
                score += 300000
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid Type 45 destroyer thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for Type 45 destroyer.")
...
def build_carrier_mesh():
    ...
    hull = trimesh.creation.box(extents=[8.6, 0.98, 0.9])
    hull.apply_translation([0.0, 0.0, 0.15])
    hull.visual.face_colors = hull_color
    parts.append(hull)

    bow = trimesh.creation.box(extents=[1.3, 0.84, 0.7])
...
resp = server.create_rodin_job(
    text_prompt='Type 45 destroyer, full ship, complete connected hull, stealth destroyer superstructure with tall radar mast, realistic naval vessel, no detached parts, clean topology',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for Type 45 destroyer.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] Type 45 destroyer package generated successfully.")
