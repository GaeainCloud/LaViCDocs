...
MODEL_NAME = "CH-53K_King_Stallion"
MODEL_NAME_I18N = "CH-53K直升机"
IMAGE_QUERY = "CH-53K King Stallion 3d render side view"
...
AGENT_DESC = "美制重型运输直升机，具备重装吊运、兵力投送与多用途支援能力。"
PREFERRED_IMAGE_URLS = [
    "https://preview.free3d.com/img/2020/02/2399387331707536926/5octwxwt.jpg",
    "https://preview.free3d.com/img/2020/02/2399387331707536926/ls0l1s7m.jpg",
    "https://preview.free3d.com/img/2020/02/2399387331707536926/e8c5os67.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    if not candidates:
        raise RuntimeError("No Bing image candidates found for CH-53K.")

    best = None
    good_kw = ("ch-53k", "king stallion", "sikorsky", "helicopter", "heavy")
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid real CH-53K thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for CH-53K.")
...
resp = server.create_rodin_job(
    text_prompt='CH-53K King Stallion military heavy-lift helicopter, realistic hard-surface helicopter, full aircraft, large fuselage, main rotor and tail rotor, high quality heavy military helicopter',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for CH-53K.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] CH-53K package generated successfully.")
