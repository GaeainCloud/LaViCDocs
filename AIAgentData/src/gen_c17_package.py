...
MODEL_NAME = "C-17_Globemaster_III"
MODEL_NAME_I18N = "C-17环球霸王III运输机"
IMAGE_QUERY = "Boeing C-17 Globemaster III 3d render side view"
...
AGENT_DESC = "美国大型战略运输机，具备远程战略空运、重装投送与多用途运输能力。"
...
def fetch_real_thumbnail(output_png):
    ...
    if not candidates:
        raise RuntimeError("No Bing image candidates found for C-17.")

    best = None
    good_kw = ("c-17", "c17", "globemaster", "globemaster iii", "boeing", "transport", "aircraft")
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid real C-17 thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for C-17.")
...
resp = server.create_rodin_job(
    text_prompt='Boeing C-17 Globemaster III military transport aircraft, realistic hard-surface aircraft, full plane, high quality large cargo jet airplane',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for C-17.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] C-17 package generated successfully.")
