...
MODEL_NAME = "Y-9G_Electronic_Warfare_Aircraft"
MODEL_NAME_I18N = "运-9G电子战飞机"
IMAGE_QUERY = "Shaanxi Y-9G electronic warfare aircraft side view"
...
AGENT_DESC = "中国电子战飞机，基于运-9平台改装，具备电子侦察、电子压制、通信干扰与战场电磁支援能力。"
PREFERRED_IMAGE_URLS = [
    "https://cdn.renderhub.com/finiask/shaanxi-y-8/shaanxi-y-8-02.jpg",
    "https://cdn.renderhub.com/finiask/shaanxi-y-8/shaanxi-y-8-03.jpg",
    "https://www.navalnews.com/wp-content/uploads/2024/09/PLAAF_Y-9LG_High_New_13_30211_Falcon_Strike.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    good_kw = ("y-9g", "y9g", "y-9", "electronic", "warfare", "shaanxi", "navalnews", "renderhub")
    ...
    if not candidates:
        raise RuntimeError("No Bing image candidates found for Y-9G.")

    for url in candidates[:80]:
        consider(url)

    if best is None:
        raise RuntimeError("Failed to download a valid real Y-9G thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for Y-9G.")
...
resp = server.create_rodin_job(
    text_prompt='Shaanxi Y-9G electronic warfare aircraft, realistic hard-surface military aircraft, full plane, high wing turboprop, side fairings, communication jamming antennas, electronic warfare mission aircraft',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for Y-9.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] Y-9G package generated successfully.")
