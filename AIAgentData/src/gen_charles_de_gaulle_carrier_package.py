...
MODEL_NAME = "Charles_de_Gaulle_R91"
MODEL_NAME_I18N = "R91 戴高乐号"
IMAGE_QUERY = "R91 Charles de Gaulle aircraft carrier 3d render side view"
...
AGENT_DESC = "法国海军核动力航空母舰，具备舰载机起降保障、远洋持续部署与编队指挥能力。"
POST_ROLL_OVERRIDE_X_DEG = 90
PREFERRED_IMAGE_URLS = [
    "https://cdn.renderhub.com/mermodels/charles-de-gaulle-r91-carrier/charles-de-gaulle-r91-carrier-03.jpg",
    "https://cdn.renderhub.com/mermodels/charles-de-gaulle-r91-carrier/charles-de-gaulle-r91-carrier-02.jpg",
    "https://cdn.renderhub.com/mermodels/charles-de-gaulle-r91-carrier/charles-de-gaulle-r91-carrier-01.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("charles de gaulle", "r91", "carrier", "aircraft carrier", "renderhub", "render", "3dmodels")
    ...
    def consider(url, strict_size=True):
        ...
        try:
            ...
            if "charles-de-gaulle-r91-carrier-03" in lower or "charles-de-gaulle-r91-carrier-02" in lower:
                score += 300000
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid Charles de Gaulle R91 carrier thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for Charles de Gaulle R91 carrier.")
...
def build_carrier_mesh():
    ...
    hull = trimesh.creation.box(extents=[9.3, 1.68, 1.20])
    hull.apply_translation([0.0, 0.0, 0.35])
    hull.visual.face_colors = hull_color
    parts.append(hull)

    bow = trimesh.creation.box(extents=[1.25, 1.28, 0.96])
    bow.apply_transform(trimesh.transformations.rotation_matrix(np.deg2rad(38), [0, 0, 1]))
    bow.apply_translation([4.68, 0.0, 0.32])
    bow.visual.face_colors = hull_color
    parts.append(bow)

    stern = trimesh.creation.box(extents=[0.95, 1.45, 0.88])
    stern.apply_translation([-4.75, 0.0, 0.28])
    stern.visual.face_colors = hull_color
    parts.append(stern)

    deck = trimesh.creation.box(extents=[9.9, 2.32, 0.12])
    deck.apply_translation([0.10, 0.12, 1.03])
    deck.visual.face_colors = deck_color
    parts.append(deck)

    angled = trimesh.creation.box(extents=[4.1, 1.12, 0.10])
    angled.apply_transform(trimesh.transformations.rotation_matrix(np.deg2rad(9), [0, 0, 1]))
    angled.apply_translation([0.45, 1.05, 1.06])
    angled.visual.face_colors = deck_color
    parts.append(angled)

    island_base = trimesh.creation.box(extents=[1.0, 0.42, 0.72])
    island_base.apply_translation([-0.40, -0.35, 1.45])
    island_base.visual.face_colors = hull_color
    parts.append(island_base)

    island_top = trimesh.creation.box(extents=[0.62, 0.28, 0.42])
    island_top.apply_translation([-0.18, -0.33, 2.02])
    island_top.visual.face_colors = hull_color
    parts.append(island_top)

    mast = trimesh.creation.box(extents=[0.15, 0.12, 0.72])
    mast.apply_translation([0.02, -0.28, 2.62])
    mast.visual.face_colors = dark
    parts.append(mast)

    deck_edge = trimesh.creation.box(extents=[6.4, 0.16, 0.18])
    deck_edge.apply_translation([0.55, -0.95, 0.95])
    deck_edge.visual.face_colors = dark
    parts.append(deck_edge)
...
resp = server.create_rodin_job(
    text_prompt='Charles de Gaulle R91 aircraft carrier, full ship, complete connected hull, catobar carrier flight deck, island superstructure starboard side, realistic naval vessel, no detached parts, clean topology',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for Charles de Gaulle R91 carrier.")
...
def apply_manual_x_rotation(glb_path, degrees):
    ...
    with open(glb_path, "wb") as f:
        f.write(trimesh.exchange.gltf.export_glb(scene))
    print(f"[glb] applied compact carrier manual X rotation {degrees:+d}° -> {glb_path}")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] Charles de Gaulle R91 carrier package generated successfully.")
