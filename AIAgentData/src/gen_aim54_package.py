...
MODEL_NAME = "AIM_54_Phoenix_Missile"
MODEL_NAME_I18N = "AIM-54不死鸟导弹"
IMAGE_QUERY = "AIM-54 Phoenix missile side view render"
...
AGENT_DESC = "AIM-54 Phoenix远程空对空导弹，具备远距拦截与比例导引攻击能力。"
MAX_RODIN_MODEL_ATTEMPTS = 3
PREFERRED_IMAGE_URLS = [
    "https://p.turbosquid.com/ts-thumb/lt/JYWsB0/74ialCKi/aim54_1/jpg/1431307794/1920x1080/fit_q87/8bcf20b477a9db511575e609e36ebc4c79fdba96/aim54_1.jpg",
    "https://p.turbosquid.com/ts-thumb/aL/glYjfx/qM/8/png/1699705814/1920x1080/fit_q87/539ae49dc04c02874f92f0a31acc4711f292f52b/8.jpg",
    "https://commons.wikimedia.org/wiki/Special:FilePath/AIM-54_PHOENIX_Sideview.png",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("aim", "54", "phoenix", "missile", "renderhub", "cgtrader", "air")
    bad_kw = (
        "logo",
        "patch",
        "wallpaper",
    ...
    def consider(url, strict_size=True):
        ...
        try:
            ...
            if "aim54_1" in lower:
                score += 1000000
            elif "/8.jpg" in lower and "turbosquid" in lower:
                score += 700000
            elif "phoenix-air-to-air-missile-3d-model" in lower:
                score += 500000
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid AIM-54 thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for AIM-54 missile.")
...
def build_missile_mesh():
    ...
    front_fin_a = trimesh.creation.box(extents=[0.20, 0.74, 0.03])
    front_fin_a.apply_translation([0.35, 0.0, 0.11])
    front_fin_a.visual.face_colors = fin_color
    parts.append(front_fin_a)

    front_fin_b = trimesh.creation.box(extents=[0.20, 0.03, 0.74])
    front_fin_b.apply_translation([0.35, 0.0, 0.11])
    front_fin_b.visual.face_colors = fin_color
    parts.append(front_fin_b)

    rear_fin_a = trimesh.creation.box(extents=[0.18, 0.55, 0.03])
    rear_fin_a.apply_translation([-1.15, 0.0, 0.11])
    rear_fin_a.visual.face_colors = fin_color
    parts.append(rear_fin_a)

    rear_fin_b = trimesh.creation.box(extents=[0.18, 0.03, 0.55])
    rear_fin_b.apply_translation([-1.15, 0.0, 0.11])
...
resp = server.create_rodin_job(
    text_prompt='AIM-54 Phoenix air-to-air missile, complete connected missile body, slender cylindrical fuselage, pointed nose cone, four mid body control fins, four rear tail fins, clean side profile, no detached parts, no floating geometry, clean topology',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for AIM-54 missile.")
...
def missile_quality_needs_fallback(glb_path):
    ...
    if extents[2] > max(extents[1] * 2.0, extents[0] * 0.55):
...
with open(r'{image_path}', 'rb') as f:
    img_bytes = f.read()
resp = server.create_rodin_job(
    text_prompt='PL-15 AAM air-to-air missile, complete connected missile body, slender cylindrical fuselage, pointed nose cone, four mid body control fins, four rear tail fins, clean side profile, no detached parts, no floating geometry, clean topology',
    images=[('.png', img_bytes)]
)
print('RESP_JSON_START')
print(json.dumps(resp))
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for PL-15 missile.")
...
def main():
    ...
    rodin_ok = False
    last_exc = None
    for attempt in range(1, MAX_RODIN_MODEL_ATTEMPTS + 1):
        try:
            print(f"[glb] rodin model attempt {attempt}/{MAX_RODIN_MODEL_ATTEMPTS}")
            generate_glb_with_blendermcp(thumb_png, glb_path)
            normalize_missile_pose_glb(glb_path)
            if missile_quality_needs_fallback(glb_path):
                print(f"[glb] rodin model attempt {attempt} rejected by quality gate")
                continue
            rodin_ok = True
            print(f"[glb] accepted rodin model on attempt {attempt}")
            break
        except Exception as exc:
            last_exc = exc
            print(f"[glb] rodin model attempt {attempt} failed: {exc}")

    if not rodin_ok:
        print(f"[glb] using fallback clean missile mesh after rodin retries: {last_exc}")
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] AIM-54 package generated successfully.")
