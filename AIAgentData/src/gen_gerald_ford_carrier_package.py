...
MODEL_NAME = "Gerald_R_Ford_Class_Carrier"
MODEL_NAME_I18N = "杰拉德·R·福特级航母"
IMAGE_QUERY = "Gerald R. Ford class aircraft carrier 3d render side view"
...
AGENT_DESC = "美国大型核动力弹射型航空母舰，具备舰载机起降保障、远洋持续部署与编队指挥能力。"
PREFERRED_IMAGE_URLS = [
    "https://www.renderhub.com/pxfpxd/ussu-gerald-r-ford/uss-gerald-r-ford-01.jpg",
    "https://cdn.renderhub.com/ennuishao/uss-gerald-r-ford/uss-gerald-r-ford-02.jpg",
    "https://cdn.3dmodels.org/wp-content/uploads/Ships/Gerald_R_Ford-class_aircraft_carrier/Gerald_R_Ford-class_aircraft_carrier_1000_0001.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("fujian", "type003", "type-003", "carrier", "aircraft carrier", "renderhub", "render")
    bad_kw = ("logo", "patch", "wallpaper", "diagram", "blueprint", "deck crew", "interior")
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid Gerald R. Ford class carrier thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for Gerald R. Ford class carrier.")
...
def build_carrier_mesh():
    ...
    hull = trimesh.creation.box(extents=[9.0, 1.65, 1.18])
...
resp = server.create_rodin_job(
    text_prompt='Gerald R. Ford class aircraft carrier, full ship, complete connected hull, flat catapult flight deck, island superstructure starboard side, realistic naval vessel, no detached parts, clean topology',
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for Gerald R. Ford class carrier.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] Gerald R. Ford class carrier package generated successfully.")
