...
MODEL_NAME = "Type_093B_Nuclear_Submarine"
MODEL_NAME_I18N = "093B型核潜艇"
IMAGE_QUERY = "Type 093B nuclear submarine 3d render side view"
...
AGENT_DESC = "中国093B型核动力攻击潜艇，具备隐蔽侦察、反潜反舰与对陆打击能力。"
PREFERRED_IMAGE_URLS = [
    "https://img-new.cgtrader.com/items/6219408/3de9d33747/large/chinese-type093-nuclear-submarine-3d-model-3de9d33747.jpg",
    "https://img-new.cgtrader.com/items/6219408/dd61e226c1/large/chinese-type093-nuclear-submarine-3d-model-dd61e226c1.jpg",
    "https://img-new.cgtrader.com/items/6219408/538449f0ec/large/chinese-type093-nuclear-submarine-3d-model-538449f0ec.jpg",
]
...
def fetch_real_thumbnail(output_png):
    ...
    best = None
    good_kw = ("093b", "type093", "submarine", "render", "3d", "cgtrader", "chinese")
    bad_kw = (
        "logo",
        "patch",
        "wallpaper",
    ...
    def consider(url, strict_size=True):
        ...
        try:
            ...
            w, h = img.size
            # Some correct CGTrader submarine renders are only ~676px on the long side.
            # Accept them so we don't fall back to noisy satellite imagery.
            if max(w, h) < 600:
                return
            score = (w * h) + (150000 if w > h else 0) + (200000 if any(k in lower for k in good_kw) else 0)
            if "chinese-type093-nuclear-submarine-3d-model-3de9d33747.jpg" in lower:
                score += 650000
            elif "chinese-type093-nuclear-submarine-3d-model-dd61e226c1.jpg" in lower:
                score += 500000
            elif "chinese-type093-nuclear-submarine-3d-model-538449f0ec.jpg" in lower:
                score += 350000
            elif "093b" in lower or "type093" in lower or "submarine" in lower:
                score += 300000
    ...
    if best is None:
        raise RuntimeError("Failed to download a valid Type 093B submarine thumbnail image.")
...
def generate_military_symbol(output_png):
    ...
    if svg is None:
        raise RuntimeError("Failed to generate military symbol for Type 093B submarine.")
...
"""
    ...
    for _ in range(60):
        ...
        raise RuntimeError("Rodin job timed out for Type 093B submarine.")
...
def main():
    ...
    if rc != 0:
        raise SystemExit(rc)
    print("[done] Type 093B submarine package generated successfully.")
